...
def optimize_posegraph_for_fragment(path_dataset, fragment_id, config):
    pose_graph_name = os.path.join(path_dataset,
                           config["template_fragment_posegraph"] % fragment_id)
    pose_graph_optimized_name = os.path.join(
        path_dataset,
...
def optimize_posegraph_for_scene(path_dataset, config):
    pose_graph_name = os.path.join(path_dataset, config["template_global_posegraph"])
    pose_graph_optimized_name = os.path.join(
...
def optimize_posegraph_for_refined_scene(path_dataset, config):
    pose_graph_name = os.path.join(path_dataset, config["template_refined_posegraph"])
    pose_graph_optimized_name = os.path.join(
