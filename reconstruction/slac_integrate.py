...
def run(config):
    ...
    slac_folder = os.path.join(path_dataset, config["subfolder_slac"])
    ...
    posegraph = o3d.io.read_pose_graph(
        os.path.join(slac_folder, config["template_optimized_posegraph_slac"]))
    ...
    if (config["save_output_as"] == "pointcloud"):
        ...
        save_pcd_path = os.path.join(slac_folder, "output_slac_pointcloud.ply")
        ...
        save_mesh_path = os.path.join(slac_folder, "output_slac_mesh.ply")
        print(f"write_triangle_mesh slac_integrate {save_mesh_path}")
