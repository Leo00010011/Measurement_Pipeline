...
from reconstruction.initialize_config import initialize_config_2
...
def pipeline(config, slac=False, debug=False):
    ...
    #NEW CONFIGURATION
    initialize_config_2(config)

    times = []
    start_time = time.time()
    make_fragments(config)
    times.append(time.time() - start_time)
    print("make_fragment FINISHED")

    start_time = time.time()    
    register_fragments(config)
    times.append(time.time() - start_time)
    print("register_fragment FINISHED")

    start_time = time.time()
    refine_registration(config)
    times.append(time.time() - start_time)
    print("refine_registration FINISHED")

    start_time = time.time()
    integrate_scene(config)
    times.append(time.time() - start_time)
    print("integrate_scene FINISHED")

    start_time = time.time()
    color_map_optimization(config)
    # times.append(time.time() - start_time)
    print("color_map_optimization FINISHED")
