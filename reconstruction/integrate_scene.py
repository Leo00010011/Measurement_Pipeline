...
def scalable_integrate_rgb_frames(path_dataset, intrinsic, config):
    ...
    [color_files, depth_files] = get_rgbd_file_list2(path_dataset,False, config['frame_step'])
    ...
    mesh_name = os.path.join(path_dataset, config["template_global_mesh"])
    print(f"integrate scene write_triangle_mesh {mesh_name}")
    o3d.io.write_triangle_mesh(mesh_name, mesh, False, True)
    print(f"integrate scene write_triangle_mesh {mesh_name}")
    traj_name = os.path.join(path_dataset, config["template_global_traj"])
