...
def run(config):
    ...
    ply_file_names = get_file_list(
        os.path.join(config["path_dataset"], config["folder_fragment"]), ".ply")
    ...
    pose_graph_fragment = o3d.io.read_pose_graph(
        os.path.join(path_dataset, config["template_refined_posegraph_optimized"]))
    ...
    if (config["method"] == "rigid"):
        ...
        key_tensor.save(
            os.path.join(slac_params.get_subfolder_name(), "ctr_grid_keys.npy"))

        value_tensor = hashmap.value_tensor()[active_buf_indices]
        value_tensor.save(
            os.path.join(slac_params.get_subfolder_name(), "ctr_grid_values.npy"))
