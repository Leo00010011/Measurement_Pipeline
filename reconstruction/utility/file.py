...
def add_if_exists(path_dataset, folder_names):
    for folder_name in folder_names:
        if exists(join(path_dataset, folder_name)):
            path = os.path.join(path_dataset, folder_name)
            return path
...
def get_rgbd_file_list2(path_dataset, masked=False, step_size = 1):
    path_color, path_depth = get_rgbd_folders(path_dataset, masked)
    color_files = get_file_list2(path_color, ".jpg",step_size) + \
        get_file_list2(path_color, ".png",step_size)
    depth_files = get_file_list2(path_depth, ".png",step_size)
    return color_files, depth_files

def get_file_list2(path,extension,step_size):
    if extension is None:
    ...
    file_list = sorted_alphanum(file_list)
    skiped_file_list = [path  for index, path in enumerate(file_list) if index % step_size == 0]
    return skiped_file_list
...
def extract_rgbd_frames(rgbd_video_file):
    ...
    """
    frames_folder = os.path.join(dirname(rgbd_video_file),
