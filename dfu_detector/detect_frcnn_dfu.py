...
class Faster_RCNN:
	...
	def prepare(self):
		...
		if self.C.network == 'resnet50':
			import dfu_detector.keras_frcnn.resnet as nn
		elif self.C.network == 'vgg':
			import dfu_detector.keras_frcnn.vgg as nn
	...
	def get_roi(self,img):
		...
		R = roi_helpers.rpn_to_roi(Y1, Y2, self.C, K.image_data_format(), overlap_thresh=0.7, max_boxes= self.C.roi_proposal)
		...
		for key in bboxes:
			...
			new_boxes, new_probs = roi_helpers.non_max_suppression_fast(bbox, np.array(probs[key]), overlap_thresh=0.3, max_boxes= self.C.roi_proposal)
...
def visualize_rois(img,x1,y1,x2,y2,color,prob):
...
def roi_process():
	...
	data_path = Path("C:\\Users\\53588\\Desktop\\Results\\bad_data")
	print('PREPARING')
...
def check_rois():
	...
	for roi in  result:
		...
		if not roi['empty']:
			visualize_rois(img,roi['x1'],roi['y1'],roi['x2'],roi['y2'],(0,0,255),roi['prob'])
		else:
...
# 	#ose another process to clean de gpu memory after detect the rois
# 	proc = multiprocessing.Process(target = roi_process)
# 	proc.start()
# 	proc.join()
# 	result = None
# 	with open('rois.json','r') as f:
